...
describe('SearchBreed Use Case', () => {
    ...
    beforeEach(() => {
        searchBreed = new searchBreedUseCase_1.default();
        jest.clearAllMocks();
    });
    it('should return breed data when search is successful', async () => {
        ...
        expect(axiosInstance.get).toHaveBeenCalledWith('https://api.thecatapi.com/v1/breeds/search?q=si', {
    ...
    it('should return empty array when search returns no results', async () => {
        ...
        expect(axiosInstance.get).toHaveBeenCalledWith('https://api.thecatapi.com/v1/breeds/search?q=nonexistentbreed', {
            headers: { 'x-api-key': 'test-api-key' }
        });
    ...
    it('should return empty array when API status is not 200', async () => {
        ...
        expect(axiosInstance.get).toHaveBeenCalledWith('https://api.thecatapi.com/v1/breeds/search?q=test', {
            headers: { 'x-api-key': 'test-api-key' }
        });
        expect(result).toEqual([]);
    });
