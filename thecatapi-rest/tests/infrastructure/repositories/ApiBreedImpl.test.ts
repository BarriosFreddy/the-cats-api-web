...
import { CAT_API_KEY, CAT_API_URL } from '../../../src/config/EnvVariables';
...
describe('ApiBreedRepository', () => {
  let apiBreedRepository: ApiBreedRepository;
  const mockHeaders = { 'x-api-key': 'test-api-key' };
  
  beforeEach(() => {
    apiBreedRepository = new ApiBreedRepository();
    jest.clearAllMocks();
  });
  ...
  describe('findById', () => {
    ...
    it('should return a breed by ID from the API', async () => {
      ...
      (axios.get as jest.Mock).mockResolvedValueOnce({ data: mockBreed });
      
      const result = await apiBreedRepository.findById(mockBreedId);
      
      // Assertions
      expect(axios.get).toHaveBeenCalledWith(
        `${CAT_API_URL}/breeds/${mockBreedId}`,
        { headers: mockHeaders }
      );
  ...
  describe('findImagesByBreedId', () => {
    ...
    it('should return breed images from the API with default limit', async () => {
      // Mock the axios.get response
      (axios.get as jest.Mock).mockResolvedValueOnce({ data: mockBreedImages });
      
      const result = await apiBreedRepository.findImagesByBreedId(mockBreedId);
      
      // Assertions
      expect(axios.get).toHaveBeenCalledWith(
        `${CAT_API_URL}/images/search`,
        { 
          headers: mockHeaders,
          params: {
