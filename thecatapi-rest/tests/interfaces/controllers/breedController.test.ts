...
import { BreedController } from '../../../src/interfaces/rest/controllers/breedController';
...
import { GetBreedImagesUseCase } from '../../../src/application/use-cases/breeds/getBreedImagesUseCase';
...
describe('BreedController', () => {
  ...
  describe('getAllBreeds', () => {
    it('should return all breeds with status 200', async () => {
      (GetAllBreedsUseCase.prototype.execute as jest.Mock).mockResolvedValue(mockBreeds);
      
      await breedController.getAllBreeds(req, res);
      
      expect(GetAllBreedsUseCase.prototype.execute).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(mockBreeds);
    });

    it('should handle errors and return 500', async () => {
      (GetAllBreedsUseCase.prototype.execute as jest.Mock).mockRejectedValue(new Error('API error'));
  ...
  describe('getBreedById', () => {
    it('should return a breed with status 200 when found', async () => {
      req.params.id = 'abys';
      (GetBreedByIdUseCase.prototype.execute as jest.Mock).mockResolvedValue(mockBreed);
      
      await breedController.getBreedById(req, res);
      
      expect(GetBreedByIdUseCase.prototype.execute).toHaveBeenCalledWith('abys');
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should return 404 when breed is not found', async () => {
      ...
      (GetBreedByIdUseCase.prototype.execute as jest.Mock).mockResolvedValue(null);
      
      await breedController.getBreedById(req, res);
      
      expect(GetBreedByIdUseCase.prototype.execute).toHaveBeenCalledWith('nonexistent');
    ...
    it('should handle errors and return 500', async () => {
      req.params.id = 'abys';
      (GetBreedByIdUseCase.prototype.execute as jest.Mock).mockRejectedValue(new Error('API error'));
  ...
  describe('getBreedImages', () => {
    it('should return breed images with status 200 with default limit', async () => {
      req.params.id = 'abys';
      (GetBreedImagesUseCase.prototype.execute as jest.Mock).mockResolvedValue(mockBreedImages);
      
      await breedController.getBreedImages(req, res);
      
      expect(GetBreedImagesUseCase.prototype.execute).toHaveBeenCalledWith('abys', 10);
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should return breed images with status 200 with custom limit', async () => {
      ...
      expect(GetBreedImagesUseCase.prototype.execute).toHaveBeenCalledWith('abys', 5);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(mockBreedImages);
    });
    ...
    it('should handle errors and return 500', async () => {
      req.params.id = 'abys';
      (GetBreedImagesUseCase.prototype.execute as jest.Mock).mockRejectedValue(new Error('API error'));
  ...
  describe('findBreed', () => {
    it('should search for breeds when query is provided', async () => {
      req.query.q = 'bengal';
      (SearchBreedUseCase.prototype.execute as jest.Mock).mockResolvedValue([mockBreeds[1]]);
      
      await breedController.findBreed(req, res);
      
      expect(SearchBreedUseCase.prototype.execute).toHaveBeenCalledWith('bengal');
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should handle errors and return 400', async () => {
      ...
      (SearchBreedUseCase.prototype.execute as jest.Mock).mockRejectedValue(error);
